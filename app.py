...
class Space(db.Model):
    ...
    bookings = db.relationship(
        'Booking', backref='space', cascade="all, delete-orphan", lazy='dynamic')
...
class Booking(db.Model):
    ...
    rental_fee = db.Column(db.Float, nullable=False, server_default='0')
...
def export_client_revenue():
    # Get the same filter values from the URL as the main page
    filter_location = request.args.get('filter_location', '')
    filter_client = request.args.get('filter_client', '')
    filter_start = request.args.get('filter_start', '')
    filter_end = request.args.get('filter_end', '')

    # Check for required date filters
    if not (filter_start and filter_end):
        return "Error: Start and end dates are required for export.", 400

    try:
        # Call the exact same RBAC-aware helper function to ensure data consistency
    ...
    column_headers = ["Client", "Location", "Prorated Revenue"]
    export_data = [column_headers]

    # Flatten the grouped data structure into a list of lists for the export
    for client_name, data in sorted(revenue_data.items()):
        for loc_name, revenue in sorted(data['locations'].items()):
            export_data.append([client_name, loc_name, revenue])

    file_name = f"client_revenue_{filter_start}_to_{filter_end}.xlsx"
    # Use make_response_from_array to guarantee column order
    return excel.make_response_from_array(export_data, 'xlsx', file_name=file_name)
# ... (All report routes would go here, calling their respective RBAC-aware helper functions) ...
...
def export_space_tracking():
    ...
    try:
        # Call the exact same helper function to ensure data consistency
        grouped_space_data, _ = get_space_tracking_data(
            filter_location, filter_space, filter_start, filter_end
        )
    except (ValueError, TypeError):
        return "Error: Invalid filter parameters for export.", 400
    ...
    # Define the column order for the Excel file
    column_headers = ["Location", "Space", "Unbooked Days", "Available Ranges"]
    export_data = [column_headers]
...
def export_income_by_space():
    ...
    # Define the column order for the Excel file
    column_headers = ["Location", "Space", "Income"]
    export_data = [column_headers]

    # Flatten the grouped data structure into a list of lists for the export
    for location_name, data in sorted(report_data.items()):
        for space_name, income in sorted(data['spaces'].items()):
            export_data.append([location_name, space_name, income])
